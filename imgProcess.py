...
my_mnist_model = models.load_model('./my_mnist_model.h5')
...
# 根据长向量找出顶点
def extractPeek(array_vals, min_vals=10, min_rect=30, max_rect=100):
    extrackPoints = []
    startPoint = None
    endPoint = None
    ...
    for point in extrackPoints:
        if (point[1] - point[0]) < min_rect or (abs(point[1] - point[0])> max_rect):
            extrackPoints.remove(point)
    return extrackPoints

# 寻找边缘，返回边框的左上角和右下角（利用直方图寻找边缘算法（需行对齐））
def findBorderHistogram(showimg):
    borders = []
    # img = cv2.imread(path, cv2.IMREAD_GRAYSCALE)
    img = cv2.cvtColor(showimg, cv2.COLOR_BGR2GRAY)
...
def findBorderContours(showimg, minArea=300, maxArea=2000):
    # img = cv2.imread(path, cv2.IMREAD_GRAYSCALE)
    img = cv2.cvtColor(showimg, cv2.COLOR_BGR2GRAY)
    img = accessBinary(img)
...
# 显示结果及边框
def showResults(showimg, borders, results=None):
    # img = cv2.imread(path)
    img = showimg
    # 绘制
    print(img.shape)
    for i, border in enumerate(borders):
        cv2.rectangle(img, border[0], border[1], (125, 125, 125))
        if results:
...
# 根据边框转换为MNIST格式
def transMNIST(showimg, borders, size=(28, 28)):
